 ...
 * of the given ecos task type. Every AI task type shares the same three script slots, so they
 * are described once here and each type only supplies its own [taskType].
 *
 * Subclasses must be registered in EcosCamundaParseListenerPlugin — a listener that is only
 * annotated with @Component never reaches the engine.
 */
abstract class AbstractAiTaskScriptsParseListener(
    ...
    override fun parseServiceTask(
        ...
        if (taskElement.attribute(BPMN_PROP_ECOS_TASK_TYPE.toCamundaKey()) != taskType) {
            return
        }
