...
@Component
class AiAgentTaskParseListener : AbstractBpmnParseListener() {
    ...
    override fun parseServiceTask(
        ...
        if (taskElement.isAiAgentTask().not()) {
            return
        }
    ...
    private fun Element.isAiAgentTask(): Boolean {
        val taskType = this.attribute(BPMN_PROP_ECOS_TASK_TYPE.toCamundaKey())
        return taskType == ECOS_TASK_AI_AGENT
    }
}
